...
The run produces a self-contained **report** rather than raw terminal output —
it documents the assumptions, the preprocessing pipeline, per-recording
provenance (channels, selected event, trial/sample counts, anything skipped),
the per-subject compression ratios, and the paired t-test with a verdict and
power warning. By default it writes a **Word document**
(`compressibility_report.docx`, via `python-docx`); pass `--report name.md` for
Markdown instead.
...
#   --report PATH   report path; .docx -> Word document, other ext -> Markdown
#                   (default: compressibility_report.docx)
